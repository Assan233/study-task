/**
 * 实现：
 * 1.分别处理TypeProperty，并对类型做转义
 * 2.遇到值是对象时，递归做JSONSchema2TS类型计算
 */
...
type JSONSchema2TS<T extends JSONSchema> =
    ...
    Omit<
        {
            [K in Exclude<keyof T['properties'], T['required'] extends string[] ? T['required'][number] : never>]?:
            T['properties'][K] extends JSONSchema ? JSONSchema2TS<T['properties'][K]> : never
        } &
        {
            [K in Extract<keyof T['properties'], T['required'] extends string[] ? T['required'][number] : never>]:
            T['properties'][K] extends JSONSchema ? JSONSchema2TS<T['properties'][K]> : never
        },
        never
    > :
